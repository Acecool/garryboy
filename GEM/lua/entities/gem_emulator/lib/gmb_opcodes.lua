...
function mt:WordDec(R1, R2)
	R1 = (R1-1 )& 0xFF
	if R1 == 0xFF then R2 = (R2-1) & 0xFF end
...
function mt:WordAdd(R1, R2)
	...
	self.Hf = (((self.H) & 0xF) + (R1 & 0xF) + (((self.L + R2) > 0xFF) and 1 or 0)) > 0xF
...
function mt:JumpSign(Val)
	if Val then
		...
		self.PC = self.PC + ((D8&127)-(D8&128)) + 2
...
function mt:Jump(Val)
	if Val then
		local A16 = (self:Read(self.PC+2)<<8)|self:Read(self.PC+1)
...
function mt:Call(Val)
	if Val then
		...
		self.SP = self.SP - 2
		self:Write(self.SP + 1, ((self.PC+3)&0xFF00)>>8)
		self:Write(self.SP    , (self.PC+3)&0xFF       )
...
function mt:Return(Val)
	if Val then
		...
		self.PC = (self:Read(self.SP + 1)<<8) | self:Read(self.SP)
		self.SP = self.SP + 2
...
function mt:ResetPC(Addr)
	self.SP = self.SP - 2
	self:Write( self.SP + 1, ((self.PC+1) & 0xFF00)>>8 )
	self:Write( self.SP    , (self.PC+1) & 0xFF )
...
function mt:ByteAdc(R1)
	...
	self.Hf = ((self.A & 0xF) + (R1 & 0x0F) + (self.Cf and 1 or 0)) > 0xF
...
function mt:ByteSub(R1)
	self.Hf = (R1 & 0xF) > (self.A & 0xF) 
	self.Cf = R1 > self.A
	
	self.A = ( self.A - R1 ) & 0xFF
...
function mt:ByteSbc(R1)
	...
	self.Hf = ((R1 & 0xF) + (self.Cf and 1 or 0) ) > (self.A & 0xF)
	self.Cf = SubVal > self.A
	
	self.A = ( self.A - SubVal ) & 0xFF
...
Operators[ 0x39 ] =  function( self ) self:WordAdd( ((self.SP>>8) & 0xFF) , (self.SP & 0xFF) ) end -- Split the SP up first

-- Add signed immediate to SP
Operators[ 0xE8 ] =  function( self )
	local D8 = self:Read(self.PC+1)
	local S8 = (D8&127)-(D8&128)-- This turns a regular 8 bit unsigned number into a signed number. 
	local SP = self.SP + S8
...
Operators[ 0x46 ] =  function( self ) self.B = self:Read((self.H<<8)|self.L); self.Cycle = 8; self.PC = self.PC + 1 end
...
Operators[ 0x4E ] =  function( self ) self.C = self:Read((self.H<<8)|self.L); self.Cycle = 8; self.PC = self.PC + 1 end
...
Operators[ 0x56 ] =  function( self ) self.D = self:Read((self.H<<8)|self.L); self.Cycle = 8; self.PC = self.PC + 1 end
...
Operators[ 0x5E ] =  function( self ) self.E = self:Read((self.H<<8)|self.L); self.Cycle = 8; self.PC = self.PC + 1 end
...
Operators[ 0x66 ] =  function( self ) self.H = self:Read((self.H<<8)|self.L); self.Cycle = 8; self.PC = self.PC + 1 end
...
Operators[ 0x6E ] =  function( self ) self.L = self:Read((self.H<<8)|self.L); self.Cycle = 8; self.PC = self.PC + 1 end
Operators[ 0x6F ] =  function( self ) self.L = self.A; self.Cycle = 4; self.PC = self.PC + 1 end

-- Load into (HL)
Operators[ 0x70 ] =  function( self ) self:Write((self.H<<8)|self.L, self.B); self.Cycle = 8; self.PC = self.PC + 1 end
Operators[ 0x71 ] =  function( self ) self:Write((self.H<<8)|self.L, self.C); self.Cycle = 8; self.PC = self.PC + 1 end
Operators[ 0x72 ] =  function( self ) self:Write((self.H<<8)|self.L, self.D); self.Cycle = 8; self.PC = self.PC + 1 end
Operators[ 0x73 ] =  function( self ) self:Write((self.H<<8)|self.L, self.E); self.Cycle = 8; self.PC = self.PC + 1 end
Operators[ 0x74 ] =  function( self ) self:Write((self.H<<8)|self.L, self.H); self.Cycle = 8; self.PC = self.PC + 1 end
Operators[ 0x75 ] =  function( self ) self:Write((self.H<<8)|self.L, self.L); self.Cycle = 8; self.PC = self.PC + 1 end

Operators[ 0x77 ] =  function( self ) self:Write((self.H<<8)|self.L, self.A); self.Cycle = 8; self.PC = self.PC + 1 end
...
Operators[ 0x7E ] =  function( self ) self.A = self:Read((self.H<<8)|self.L); self.Cycle = 8; self.PC = self.PC + 1 end
...
Operators[ 0x36 ] =  function( self ) self:Write((self.H<<8)|self.L, self:Read(self.PC+1)); self.Cycle = 12; self.PC = self.PC + 2 end
...
Operators[ 0x22 ] = function( self )
	...
	self:Write( (self.H<<8)|self.L, self.A )
...
Operators[ 0x2A ] = function( self )
	...
	self.A = self:Read( (self.H<<8)|self.L )
...
Operators[ 0x86 ] = function( self ) self:ByteAdd( self:Read( (self.H<<8)|self.L) ); self.Cycle = 8 end
...
Operators[ 0x8E ] = function( self ) self:ByteAdc( self:Read( (self.H<<8)|self.L ) ); self.Cycle = 8 end
...
Operators[ 0x96 ] = function( self ) self:ByteSub( self:Read( (self.H<<8)|self.L ) ); self.Cycle = 8 end
...
Operators[ 0x9E ] = function( self ) self:ByteSbc( self:Read( (self.H<<8)|self.L ) ); self.Cycle = 8 end
...
Operators[ 0xA6 ] = function( self ) self:ByteAnd( self:Read((self.H<<8)|self.L) ); self.Cycle = 8 end
...
Operators[ 0xAE ] = function( self ) self:ByteXor( self:Read((self.H<<8)|self.L) ); self.Cycle = 8 end
...
Operators[ 0xB6 ] = function( self ) self:ByteOr( self:Read((self.H<<8)|self.L) ); self.Cycle = 8 end
...
Operators[ 0xBE ] = function( self ) self:ByteCmp( self:Read((self.H<<8)|self.L) ); self.Cycle = 8 end
...
Operators[ 0x34 ] = function( self )
	local R1 = self:Read((self.H<<8)|self.L)
	local R1 = self:ByteInc(R1)
	self:Write( (self.H<<8)|self.L, R1 )
...
Operators[ 0xF5 ] = function( self )
	self.F = 0
	if self.Cf then self.F = self.F|16 end
	if self.Hf then self.F = self.F|32 end
	if self.Nf then self.F = self.F|64 end
	if self.Zf then self.F = self.F|128 end
...
Operators[ 0xF1 ] = function( self )
	...
	if self.F&16 == 16 then self.Cf = true else self.Cf = false end
	if self.F&32 == 32 then self.Hf = true else self.Hf = false end
	if self.F&64 == 64 then self.Nf = true else self.Nf = false end
	if self.F&128 == 128 then self.Zf = true else self.Zf = false end
end
...
Operators[ 0x31 ] = function( self ) 
	self.SP = ( self:Read( self.PC + 2 ) << 8 ) | self:Read(self.PC + 1)
...
Operators[ 0x08 ] = function( self ) 
	...
	local A16 = (self:Read(self.PC+2)<<8)|self:Read(self.PC+1)
	local SPhi = (self.SP&0xFF00)>>8
	local SPlo = ( self.SP & 0xFF )
...
Operators[ 0x27 ] = function( self ) 
	...
	if self.Nf then
		if (self.A & 0x0F) > 9 or self.Hf then
			self.A = (self.A - 6)
			
			if (self.A & 0xF0) == 0xF0 then self.Cf = true end
		end
		
		if (self.A & 0xF0) > 0x90 or self.Cf then self.A = (self.A - 0x60); self.Cf = true end

	else
		if (self.A & 0xF) > 9 or self.Hf then
			self.A = (self.A + 0x06)
		end
...
Operators[ 0x17 ] = function( self ) 
	local Bit7 = self.A&128 == 128
	
	self.A = ((self.A<<1) & 0xFF) | (self.Cf and 1 or 0)
...
Operators[ 0x1F ] = function( self )
	local Bit0 = self.A&1 == 1

	self.A = ((self.A>>1) & 0xFF) | (self.Cf and 128 or 0)
...
Operators[ 0x07 ] = function( self )
	...
	self.A = ((self.A<<1) & 0xFF) | (Bit7 and 1 or 0)
...
Operators[ 0x0F ] = function( self )
	...
	self.A = ((self.A>>1) & 0xFF) | (Bit0 and 128 or 0)
